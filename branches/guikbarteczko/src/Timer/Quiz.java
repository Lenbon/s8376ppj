...
public class Quiz {
    ...
    private final String[] CAP = { "Praga", "Bratysława", "Moskwa", "Berlin",
            "Kijów", "Wilno", "Mińsk" };
    ...
    public Quiz() {
        ...
        JOptionPane.showMessageDialog(null,
                "Podaj stolice lądowych sąsiadów Polski");
        ...
        while (count < CAP.length) { // dopóki nie podano wszystkich stolic
            String input = JOptionPane.showInputDialog("Odpowiedzi: " + count
                    + "/" + n + "\n" + askMsg);
            if (input == null)
                break;
            if (isOk(input))
                count++; // jeżeli ta odpowiedź prawidłowa
        }
        System.exit(0);
    }
