...
public class Zadanie30 {
    public static void main(String[] args) {
        ...
        do {
            try {
                n = Integer.parseInt(JOptionPane
                        .showInputDialog("Podaj liczbę n:"));
                if (n <= 0 || n > 25) {
                    throw new Exception(
                            "n musi być w większe od 0 i mniejsze od 25");
                }
                flag = true;
            } catch (Exception e) {
                JOptionPane.showMessageDialog(null,
                        "n nie jest liczbą większą od 0");
                flag = false;
            }
        } while (flag == false);
        ...
        do {
            try {
                input = JOptionPane
                        .showInputDialog("Podaj zbior liczb (format: x,y;z,w):");
                ...
                for (String val : array) {
                    ...
                    if (x < 0 || y < 0 || x > n || y > n) {
                        throw new Exception("Liczby wykraczają poza zbiór 0 - "
                                + n);
                    }
                ...
                JOptionPane.showMessageDialog(null,
                        "Podany ciąg znaków nie jest poprawny");
                flag = false;
            }
        } while (flag == false);
        ...
        System.out
                .println("W zbiorze " + (!flagRelReturn ? "NIE " : "")
                        + "występuje relacja zwrotna " + "oraz "
                        + (!flagRelSym ? "NIE " : "")
                        + "występuje relacja symetryczna");
    }
}
