			...
			try {
				...
				while (inputFlag == false) {
					try {
						...
						if (simpleDateFormat.format(dateNow).toString().substring(0, 8).equals(simpleDateFormat.format(dateModel).toString().substring(0, 8))) {
							// do nothing
						} else {
						...
						if (inputDouble % 100 != 0) {
							throw new Exception("Bankomat wypłaca jedynie banknoty o nominale 100" + faceValue);
						}
						...
						account.put("balance", "" + (Double.parseDouble(account.get("balance")) - inputDouble));
