...
public class Bus extends EngineVehicle {
	...
	public void passengersEmbarking(int number) throws Exception {
		if (!ztm && number > seatsAvalible) {
			...
			seatsAvalible = 0;
			throw new Exception("Nie ma miejsc dla wszystkich pasażerów. Nie wsiadło " + atTheBusStop + " pasażerów.");
		} else {
	...
	public String toString() {
		...
		return "Pojazd " + currNr + " (" + getRegisterNumber() + "), właścicielem którego jest " + s + " - " +
			states[state] + ";\nPasażerowie/miejsca: " + ((seatsAvalible - seats) * -1) + "/" + seats
			;
	}
