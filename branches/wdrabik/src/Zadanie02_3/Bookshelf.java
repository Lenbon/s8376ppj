...
public class Bookshelf {
	...
	public void insert(Book book) throws Exception {
		if (shelf.size() >= size) {
			throw new Exception("Nie mogę wstawić książki na półkę (Półka " + name + ") - brak miejsca");
		}
		...
		for (Bookshelf val : storage) {
			for (Book subVal : val.getBooks()) {
				if (book.getAuthor().equals(subVal.getAuthor()) & book.getTitle().equals(subVal.getTitle())) {
					throw new Exception("Nie mogę wstawić książki na półkę (Półka " + name + ") - jest już na półce: Półka " + val.getName());	
				}
			}
		}
	...
	public void remove(Book book) throws Exception {
		...
		throw new Exception("Nie mogę zdjąć książki z półki (Półka " + name + ") - nie jest na tej półce");
	}
